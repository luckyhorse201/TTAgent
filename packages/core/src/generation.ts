    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.LMSTUDIO: {
                ...
                console.log("@@@@@context before aiGenerateText in generateText: ",context);
                console.log("=============================================================")
                ...
                console.log("@@@@@reponse from generateText: ",openaiResponse);
                console.log("=============================================================")
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.OPENROUTER: {
                ...
                const regex = /\(just now\)\s+\[[^\]]+]\s+([^:]+):\s+([\s\S]*?)\s*\(([^)]+)\)/g;

                let match: RegExpExecArray | null;
                let lastMatch: RegExpExecArray | null = null;

                while ((match = regex.exec(context)) !== null) {
                lastMatch = match; // store the last (just now) match
                }

                if (lastMatch) {
                    const user = lastMatch[1].trim();
                    const justNowMessage = lastMatch[2].trim();
                    const action = lastMatch[3].trim();                

                    console.log(`@@@@@just now part: ${justNowMessage}`);
                    if (justNowMessage.includes("/start")){
                        const cleaned = `Welcome to the SonicXBT Telegram Agent!\n\nI’m here to provide you with real-time trading insights, market analysis, and AI-powered precision.\n\nAsk me anything—how can I assist you today?`;
                        console.log(cleaned);
                        
                        response = "\n```json\n" + `{ \"user\": \"${user}\", \"text\": \"${cleaned}\", \"action\": \"NONE\"}` + "\n```";
                        elizaLogger.debug("Received response from OpenRouter model. response: ", response);
                    } else{
                        const { text: openrouterResponse } = await aiGenerateText({
                            model: openrouter.languageModel(model),
                            prompt: justNowMessage,
                            temperature: temperature,
                            system:
                                runtime.character.system ??
                                settings.SYSTEM_PROMPT ??
                        ...
                        const removeReferences = (text: string): string => {
                            // Match patterns like [1], [12], [Doe, 2021], etc. at the END of a sentence
                            // return text.replace(/\s*\[[^\]]+\](?=\s*[.?!])/g, "");
                            
                            // Handles [1][2][3], [@user], [#123], etc.
                            return text.replace(/(\[[^\]]*\])+/g, "");
                        };
                        
    
                        const cleaned = removeReferences(openrouterResponse);
                    ...
                    console.log("No (just now) message found.");
                }
...
export async function generateTextTwitter({
    runtime,
    context,
    modelClass,
    tools = {},
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.OPENROUTER: {
                ...
                function cutBeforeLastMention(text: string, mention: string = "@darkhorse201199"): string {
                    const index = text.lastIndexOf(mention);
                    if (index === -1) return text; // mention not found
                    return text.slice(index + mention.length).trim();
                }
                const cut_before_last_mention = cutBeforeLastMention(context);
                const complete_prompt = "Very simply respond without reference: " + cut_before_last_mention;
                console.log("@@@@@@complet prompt:", complete_prompt);
                ...
                // elizaLogger.debug("Received response from OpenRouter model. response: ", response);
                response = openrouterResponse;
                console.log("@@@@@@openrouter response for twitter reply: ",response);
                break;
            }
...
export async function generateShouldRespond({
    ...
    while (true) {
        try {
            ...
            console.log("@@@@@@@@@context before generateText in generateShouldRespond: ", context);
            console.log("=========================================================================");
...
export async function generateMessageResponse({
    ...
    while (true) {
        try {
            ...
            console.log("@@@@@@@@@context before generateText in generateMessageResponse: ", context);
            console.log("=========================================================================");
            const response = await generateText({
                runtime,
                context,
            ...
            console.log("@@@@@@@@@context after generateText in generateShouldRespond: ", response);
            console.log("=========================================================================");
            console.log("@@@@@@@@@context after generateText in generateShouldRespond: ", response);
            console.log("=========================================================================");
 ...
 * @returns The completed message.
 */
export async function generateMessageResponseSimple({
    runtime,
    context,
    modelClass,
}: {
 ...
 * @returns The completed message.
 */
export async function generateMessageResponseTwitter({
    runtime,
    context,
    modelClass,
}: {
    ...
    while (true) {
        try {
            elizaLogger.log("@@@@@@Generating Twitter reply message response..");

            const response = await generateTextTwitter({
                runtime,
                context,
                modelClass,
            });
